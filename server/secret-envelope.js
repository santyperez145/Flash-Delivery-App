...
const paymentOAuthKey=crypto.createHash("sha256").update(config.paymentMarketplace.encryptionKey,"utf8").digest();
...
export function encryptPaymentOAuthToken(value){const iv=crypto.randomBytes(12),cipher=crypto.createCipheriv("aes-256-gcm",paymentOAuthKey,iv);cipher.setAAD(Buffer.from("flash-payment-oauth-v1"));const ciphertext=Buffer.concat([cipher.update(String(value),"utf8"),cipher.final()]);return["v1",iv.toString("base64url"),cipher.getAuthTag().toString("base64url"),ciphertext.toString("base64url")].join(".");}
export function decryptPaymentOAuthToken(envelope){const[version,iv,tag,ciphertext]=String(envelope||"").split(".");if(version!=="v1"||!iv||!tag||!ciphertext)throw new Error("Invalid payment OAuth envelope");const decipher=crypto.createDecipheriv("aes-256-gcm",paymentOAuthKey,Buffer.from(iv,"base64url"));decipher.setAAD(Buffer.from("flash-payment-oauth-v1"));decipher.setAuthTag(Buffer.from(tag,"base64url"));return Buffer.concat([decipher.update(Buffer.from(ciphertext,"base64url")),decipher.final()]).toString("utf8");}
