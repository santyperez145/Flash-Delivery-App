...
async function calculate(client, zonePublicId) {
  const row = (
    await client.query(
      `SELECT z.id,z.public_id,z.name,c.slug city,p.min_fresh_drivers,p.min_active_branches,p.min_completed_jobs_7d,p.max_cancellation_percent,p.max_urgent_tickets,
    ...
    FROM service_zones z JOIN cities c ON c.id=z.city_id JOIN zone_readiness_policies p ON p.zone_id=z.id WHERE z.public_id=$1`,
      [zonePublicId],
    )
  ).rows[0];
