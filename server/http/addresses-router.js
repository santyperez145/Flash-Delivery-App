...
//
// Lo que hace explícito este archivo es la **lista de dependencias**. Un grupo
// de rutas necesita siete cosas del núcleo, y hasta que esas siete no sean
// módulos propios, cada router nuevo va a recibirlas por parámetro. Eso no es
// un defecto del patrón: es la medida de cuánto núcleo queda por extraer, y
// está escrito acá en lugar de disperso en 265 líneas de un archivo de 9.500.
//
...
export function createAddressesRouter({ requireAuth, readDb, audit }) {
  ...
  router.put("/api/addresses/:addressId", requireAuth, async (req, res) => {
    const parsed = parseOrFail(addressSchema, req.body || {});
    if (!parsed.ok) return fail(res, 400, parsed.message);
    if (!usesPostgresAuth()) {
      const db = readDb();
  ...
  router.patch("/api/addresses/:addressId/default", requireAuth, async (req, res) => {
    if (!usesPostgresAuth()) {
      const db = readDb();
      const address = (db.addresses || []).find(
        (entry) => entry.id === req.params.addressId && entry.userId === req.auth.userId,
  ...
  router.delete("/api/addresses/:addressId", requireAuth, async (req, res) => {
    if (!usesPostgresAuth()) {
      const db = readDb();
