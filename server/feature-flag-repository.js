...
const bucketFor = (key, userId) =>
  Number.parseInt(
    crypto
      .createHmac("sha256", config.featureFlagSalt)
      .update(`${key}:${userId}`)
      .digest("hex")
      .slice(0, 8),
    16,
...
const mapFlag = (row) => ({
  ...
  allowedRoles: rolesOf(row.allowed_roles),
...
export async function evaluateFeatureFlags({ userId, roles }) {
  const result = await postgresPool.query(
    `SELECT f.*,c.slug city_slug,u.city_id user_city_id FROM feature_flags f
    LEFT JOIN cities c ON c.id=f.city_id CROSS JOIN users u WHERE u.public_id=$1 ORDER BY f.key`,
    [userId],
  );
  const now = Date.now();
  return Object.fromEntries(
    result.rows.map((row) => {
      ...
      const timeMatch =
        (!row.starts_at || row.starts_at.getTime() <= now) &&
        (!row.ends_at || row.ends_at.getTime() > now);
      const active = Boolean(
        row.enabled &&
          roleMatch &&
          cityMatch &&
          timeMatch &&
          bucketFor(row.key, userId) < row.rollout_percentage,
      );
...
export async function getFeatureFlags() {
  const result = await postgresPool.query(
    `SELECT f.*,c.slug city_slug FROM feature_flags f LEFT JOIN cities c ON c.id=f.city_id ORDER BY f.key`,
  );
...
export async function updateFeatureFlag({ publicId, changes }) {
  const row = (
    await postgresPool.query(
      `UPDATE feature_flags SET enabled=COALESCE($2,enabled),rollout_percentage=COALESCE($3,rollout_percentage),
    ...
    WHERE public_id=$1 RETURNING id`,
      [
        ...
        changes.rolloutPercentage ?? null,
        changes.allowedRoles ?? null,
        Object.hasOwn(changes, "startsAt"),
        changes.startsAt ?? null,
        Object.hasOwn(changes, "endsAt"),
        changes.endsAt ?? null,
        changes.variant === undefined ? null : JSON.stringify(changes.variant),
      ],
    )
  ).rows[0];
