...
Object.assign(openApiDocument.paths, {
  ...
  "/api/support/tickets/{ticketId}/messages":{post:{tags:["Support"],operationId:"addSupportMessage",summary:"Agregar mensaje a un ticket visible",security:[{bearerAuth:[]}],parameters:[{name:"ticketId",in:"path",required:true,schema:{type:"string",minLength:3}}],requestBody:body("SupportMessageRequest"),responses:{200:success({$ref:"#/components/schemas/SupportTicketResponse"}),400:errorResponses[400],401:errorResponses[401],403:bearerErrors[403],404:{description:"Ticket inexistente o ajeno",content:json},429:errorResponses[429],503:{description:"Soporte real requiere PostgreSQL",content:json}}}},
...
Object.assign(openApiDocument.components.schemas, {
  ...
  SupportTicket:{type:"object",required:["id","category","priority","subject","status"],properties:{id:{type:"string"},category:{enum:["food","ride","shipment","payment","account","safety","other"]},priority:{enum:["low","normal","high","urgent"]},subject:{type:"string"},status:{enum:["open","waiting_customer","waiting_operations","resolved","closed"]},messages:{type:"array",items:{type:"object"}},createdAt:{type:"string",format:"date-time"},updatedAt:{type:"string",format:"date-time"}}},
