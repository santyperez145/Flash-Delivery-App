...
Object.assign(openApiDocument.paths, {
  ...
  "/api/payment-provider/client-configuration": { get: { tags: ["Payments"], operationId: "getPaymentClientConfiguration", summary: "Obtener configuración pública para tokenización", security: [{ bearerAuth: [] }], responses: { 200: success({ $ref: "#/components/schemas/PaymentClientConfigurationResponse" }), ...bearerErrors } } },
...
Object.assign(openApiDocument.components.schemas, {
  ...
  PaymentClientConfigurationResponse: { type: "object", required: ["ok", "requestId", "provider", "publicKey", "cardDataHandling"], properties: { ok: { const: true }, requestId: { type: "string" }, provider: { enum: ["mercadopago", "disabled"] }, publicKey: { type: ["string", "null"] }, cardDataHandling: { const: "provider_tokenization_only" } } },
  ...
  MerchantPaymentConnection:{type:"object",required:["provider","externalAccountId","liveMode","connectedAt","status"],properties:{provider:{const:"mercadopago"},externalAccountId:{type:"string"},liveMode:{type:"boolean"},scope:{type:["string","null"]},connectedAt:{type:"string",format:"date-time"},tokenExpiresAt:{type:["string","null"],format:"date-time"},status:{enum:["connected","revoked"]}}},
