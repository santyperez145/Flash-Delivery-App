...
import {allRoles,adminOnly,classifyRealtimeAudience,jobEntityTypes} from "./realtime-audience.js";
...
async function ownerOfDriver(entityId){
  const row=(await postgresPool.query("SELECT u.public_id FROM drivers d JOIN users u ON u.id=d.user_id WHERE d.public_id=$1",[entityId])).rows[0];
  return row?[row.public_id]:[];
}

async function ownerOfMerchant(entityId){
  const row=(await postgresPool.query("SELECT u.public_id FROM merchants m JOIN users u ON u.id=m.owner_id WHERE m.public_id=$1",[entityId])).rows[0];
  return row?[row.public_id]:[];
}

async function ownerOfSupportTicket(entityId){
  const row=(await postgresPool.query("SELECT u.public_id FROM support_tickets t JOIN users u ON u.id=t.user_id WHERE t.public_id=$1",[entityId])).rows[0];
  return row?[row.public_id]:[];
}
...
async function ownerOfAddress(entityId){
  ...
  const row=(await postgresPool.query("SELECT u.public_id FROM addresses a JOIN users u ON u.id=a.user_id WHERE a.id=$1::uuid",[entityId])).rows[0];
  return row?[row.public_id]:[];
}

async function participantsOfJob(entityId){
  const row=(await postgresPool.query(`SELECT customer.public_id customer_id,merchant_owner.public_id merchant_id,driver_user.public_id driver_id
...
async function resolveAudience({type,entityType,entityId,actorPublicId}){
  ...
  if(kind==="unclassified"||!entityId)return{users:[],roles:adminOnly,outcome:"unclassified"};
...
export async function persistPostgresRealtimeEvent({type,entityType=null,entityId=null,action=null,requestId=null,actorPublicId=null}){const audience=await resolveAudience({type,entityType,entityId,actorPublicId});observeRealtimeAudience({entityType,outcome:audience.outcome});const row=(await postgresPool.query(`INSERT INTO realtime_events(public_id,type,entity_type,entity_id,action,request_id,actor_public_id,audience_user_ids,audience_roles)
