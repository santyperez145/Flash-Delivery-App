...
app.get("/api/payment-provider/client-configuration",requireAuth,requireAnyRole("customer"),async(req,res)=>{res.set("Cache-Control","no-store, private");const enabled=config.paymentMarketplace.provider==="mercadopago"&&Boolean(config.paymentMarketplace.publicKey),merchantId=String(req.query.merchantId||"").slice(0,100);let merchantReady=false;if(enabled&&merchantId&&postgresPool)merchantReady=Boolean((await postgresPool.query(`SELECT 1 FROM merchant_payment_connections c JOIN merchants m ON m.id=c.merchant_id WHERE m.public_id=$1 AND c.provider='mercadopago' AND c.revoked_at IS NULL AND c.access_token_ciphertext IS NOT NULL AND (c.token_expires_at IS NULL OR c.token_expires_at>now())`,[merchantId])).rowCount);return ok(res,{provider:enabled?"mercadopago":"disabled",publicKey:enabled?config.paymentMarketplace.publicKey:null,merchantReady,cardDataHandling:"provider_tokenization_only"});});
