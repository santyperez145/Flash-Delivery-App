...
app.get("/api/payment-provider/client-configuration",requireAuth,requireAnyRole("customer"),(req,res)=>{res.set("Cache-Control","no-store, private");const enabled=config.paymentMarketplace.provider==="mercadopago"&&Boolean(config.paymentMarketplace.publicKey);return ok(res,{provider:enabled?"mercadopago":"disabled",publicKey:enabled?config.paymentMarketplace.publicKey:null,cardDataHandling:"provider_tokenization_only"});});
