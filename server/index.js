...
app.get("/api/operations/restaurants", requireAuth, requireAnyRole("admin"), async (req, res) => {
...
app.get("/api/operations/audit-events", requireAuth, requireAnyRole("admin"), async (req, res) => {
  const limit = Math.min(100, Math.max(1, Number(req.query.limit) || 50)),
    query = String(req.query.q || "").slice(0, 100),
    cursor = parseOperationsTimestampCursor(req.query.cursor, "occurredAt", usesPostgresCommerce());
  if (cursor === false) return fail(res, 400, "Cursor operativo inválido");
  try {
    res.set("Cache-Control", "no-store, private");
    if (!usesPostgresCommerce()) {
...
app.patch("/api/zones/:zoneId", requireAuth, requireAnyRole("admin"), async (req, res) => {
