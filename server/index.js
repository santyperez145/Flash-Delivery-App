...
app.get("/api/driver/earnings", requireAuth, requireAnyRole("driver"), async (req,res)=>{
  try {
    if (!usesPostgresCommerce()) {
      ...
      const summarize=(start,end)=>{const scoped=entries.filter((entry)=>new Date(entry.createdAt)>=start&&new Date(entry.createdAt)<end);return{amount:scoped.reduce((sum,entry)=>sum+entry.amount,0),serviceEarnings:scoped.filter((entry)=>!["tip","adjustment"].includes(entry.category)).reduce((sum,entry)=>sum+entry.amount,0),tips:scoped.filter((entry)=>entry.category==="tip").reduce((sum,entry)=>sum+entry.amount,0),adjustments:scoped.filter((entry)=>entry.category==="adjustment").reduce((sum,entry)=>sum+entry.amount,0),services:scoped.filter((entry)=>!["tip","adjustment"].includes(entry.category)).length,onlineSeconds:null,activeSeconds:null,periodStart:start.toISOString(),periodEnd:end.toISOString()};};
      ...
      const days=[];for(let cursor=new Date(weekStart);cursor<=dayStart;cursor=new Date(cursor.getFullYear(),cursor.getMonth(),cursor.getDate()+1)){const end=new Date(cursor);end.setDate(end.getDate()+1);const summary=summarize(cursor,end);days.push({date:`${cursor.getFullYear()}-${String(cursor.getMonth()+1).padStart(2,"0")}-${String(cursor.getDate()).padStart(2,"0")}`,amount:summary.amount,serviceEarnings:summary.serviceEarnings,tips:summary.tips,adjustments:summary.adjustments,services:summary.services,onlineSeconds:null,activeSeconds:null});}
      res.set("Cache-Control","no-store, private");return ok(res,{earnings:{driverId:driver.id,currency:"ARS",timezone:"America/Argentina/Buenos_Aires",source:"sqlite-test-fallback",walletBalance:Number(user?.wallet||0),today:summarize(dayStart,dayEnd),week:summarize(weekStart,weekEnd),days,recent:entries.slice(0,100),timeTracking:{status:"unavailable",reason:"postgres_required"},cashout:{status:"not_configured",reason:"external_payout_provider_required"}}});
    }
