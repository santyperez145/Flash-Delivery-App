...
export async function acceptDispatchOffer(client,{jobPublicId,driverPublicId,actorUserId,status="driver_assigned"}){
  const offer=(await client.query(`SELECT o.id,o.job_id,o.driver_id,j.kind FROM dispatch_offers o JOIN jobs j ON j.id=o.job_id JOIN drivers d ON d.id=o.driver_id
    WHERE j.public_id=$1 AND d.public_id=$2 AND o.status='pending' AND o.expires_at>now()
      AND (COALESCE(j.metadata->>'subtype','')<>'food_order' OR j.status='ready_for_pickup')
      AND (j.scheduled_for IS NULL OR j.scheduled_for<=now()+interval '15 minutes') FOR UPDATE OF o`,[jobPublicId,driverPublicId])).rows[0];
  ...
  const changed=(await client.query(`UPDATE jobs SET driver_id=$1,status=$2,version=version+1,updated_at=now()
    WHERE id=$3 AND driver_id IS NULL AND status NOT IN('completed','cancelled')
      AND (COALESCE(metadata->>'subtype','')<>'food_order' OR status='ready_for_pickup') RETURNING id,customer_id`,[offer.driver_id,status,offer.job_id])).rows[0];
