...
export async function getPostgresRideDestinations(userPublicId, limit = 8) {
  const result = await postgresPool.query(
    `SELECT h.id, h.label, h.formatted_address, h.use_count, h.last_used_at,
       ST_Y(h.location::geometry) lat, ST_X(h.location::geometry) lng
     FROM ride_destination_history h
     JOIN users u ON u.id=h.user_id
     WHERE u.public_id=$1
     ORDER BY h.last_used_at DESC LIMIT $2`,
    [userPublicId, limit],
  );
...
export async function recordPostgresRideDestination({ userPublicId, label, address, lat, lng }) {
  ...
  try {
    ...
    const row = (
      await client.query(
        `INSERT INTO ride_destination_history(
          user_id, address_key, label, formatted_address, location
        ) VALUES($1, $2, $3, $4, ST_SetSRID(ST_MakePoint($6, $5), 4326)::geography)
         ON CONFLICT(user_id, address_key) DO UPDATE SET
           label=excluded.label,
           formatted_address=excluded.formatted_address,
           location=excluded.location,
           use_count=ride_destination_history.use_count+1,
           last_used_at=now(),
           updated_at=now()
         RETURNING id, label, formatted_address, use_count, last_used_at,
           ST_Y(location::geometry) lat, ST_X(location::geometry) lng`,
