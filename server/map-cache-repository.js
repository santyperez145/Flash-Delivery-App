...
export async function getCachedMapResponse({ kind, key }) {
  ...
  const result = await postgresPool.query(
    `UPDATE map_provider_cache SET hit_count=hit_count+1,updated_at=now()
    WHERE cache_key=$1 AND kind=$2 AND expires_at>now() RETURNING provider,payload,expires_at`,
    [key, kind],
  );
...
export async function getStaleCachedMapResponse({ kind, key, maxStaleSeconds }) {
  if (!postgresPool) return null;
  const result = await postgresPool.query(
    `SELECT provider,payload,expires_at FROM map_provider_cache
    WHERE cache_key=$1 AND kind=$2 AND expires_at>now()-$3*interval '1 second'`,
    [key, kind, maxStaleSeconds],
  );
  const row = result.rows[0];
  return row
    ? {
...
export async function putCachedMapResponse({ kind, key, provider, payload, ttlSeconds }) {
  ...
  await postgresPool.query(
    ...
    ON CONFLICT(cache_key) DO UPDATE SET kind=excluded.kind,provider=excluded.provider,payload=excluded.payload,
      expires_at=excluded.expires_at,hit_count=0,updated_at=now()`,
    [key, kind, provider, payload, ttlSeconds],
  );
}
