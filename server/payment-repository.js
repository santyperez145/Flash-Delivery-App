...
export async function recordPaymentWebhook({
  ...
  await postgresPool.query("UPDATE webhook_events SET processed_at=now() WHERE id=$1", [
    inserted.rows[0].id,
  ]);
  ...
  try {
    ...
    for (const row of stale)
      await upsertCase(client, {
        ...
        provider: row.provider,
        caseType: "stale_intent",
        severity: row.status === "authorized" ? "high" : "medium",
        entityType: "payment_intent",
        entityId: row.id,
        externalReference: row.provider_intent_id,
        summary: `Intento ${row.status} sin actualización por más de 30 minutos`,
        details: {
          status: row.status,
    ...
    for (const row of captureMismatch)
      await upsertCase(client, {
        fingerprint: `capture:${row.id}`,
        provider: row.provider,
        caseType: "capture_mismatch",
        severity: "critical",
        entityType: "payment_intent",
        entityId: row.id,
        externalReference: row.provider_intent_id,
        summary: "El estado del intento no coincide con el importe capturado",
        details: {
          status: row.status,
          amountCents: Number(row.amount_cents),
    ...
    for (const row of refundMismatch)
      await upsertCase(client, {
        ...
        entityId: row.id,
        externalReference: row.provider_intent_id,
        summary: "Los reintegros confirmados superan el importe capturado",
        details: {
          capturedAmountCents: Number(row.captured_amount_cents),
          refundedAmountCents: Number(row.refunded_cents),
          currency: row.currency,
        },
      });
    ...
    for (const row of webhookFailures) {
      const orphan = row.signature_valid && !row.processing_error,
        kind = orphan ? "orphan_webhook" : "webhook_failure";
      await upsertCase(client, {
        fingerprint: `webhook:${row.id}:${kind}`,
        provider: row.provider,
        caseType: kind,
        severity: orphan ? "high" : row.signature_valid ? "high" : "medium",
        entityType: "webhook_event",
        entityId: row.id,
        externalReference: row.provider_event_id,
        summary: orphan
          ? "Webhook firmado sin intento de pago asociado"
          : "Webhook rechazado o con error de procesamiento",
        details: {
          eventType: row.event_type,
          signatureValid: row.signature_valid,
          receivedAt: row.received_at,
          providerIntentId: row.provider_intent_id || null,
          hasProcessingError: Boolean(row.processing_error),
        },
      });
    }
...
export async function resolvePaymentReconciliationCase({
  ...
  if (!row) throw Object.assign(new Error("Caso abierto no encontrado"), { status: 404 });
  return mapCase(row);
}
