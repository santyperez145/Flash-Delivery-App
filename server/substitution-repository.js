...
const map = (row) => ({
  ...
  reason: row.reason,
  original: {
    id: row.original_public_id || row.original_snapshot.publicId,
    name: row.original_snapshot.name,
    unitPrice: money(row.original_unit_price_cents),
  },
  replacement: {
    id: row.replacement_public_id || row.replacement_snapshot.publicId,
    name: row.replacement_snapshot.name,
    unitPrice: money(row.replacement_unit_price_cents),
  },
...
export async function proposeOrderSubstitution({
  ...
  try {
    ...
    const row = (
      await client.query(
        `SELECT j.id job_id,j.customer_id,ji.id job_item_id,ji.quantity,ji.unit_price_cents original_price,ji.name original_name,
    ...
    if (!row)
      throw Object.assign(new Error("Pedido, producto o comercio no disponible para sustituir"), {
        status: 404,
      });
    const originalEffectivelyAvailable =
      row.original_available &&
      row.original_branch_available &&
      (row.original_branch_quantity === null ||
        Number(row.original_branch_quantity) >= Number(row.quantity));
    const replacementEffectivelyAvailable =
      row.replacement_available &&
      row.replacement_branch_available &&
      (row.replacement_branch_quantity === null ||
        Number(row.replacement_branch_quantity) >= Number(row.quantity));
    if (originalEffectivelyAvailable)
      throw Object.assign(
        new Error("Marca el producto original sin stock en la sucursal antes de sustituirlo"),
        { status: 409 },
      );
    if (!replacementEffectivelyAvailable)
      throw Object.assign(
        new Error("El reemplazo no tiene disponibilidad suficiente en la sucursal del pedido"),
        { status: 409 },
      );
    if (
      row.original_id === row.replacement_id ||
      Number(row.replacement_price) > Number(row.original_price)
    )
      throw Object.assign(new Error("El reemplazo debe ser distinto y de precio igual o menor"), {
        status: 409,
      });
    const inserted = (
      await client.query(
        ...
        [
          ...
          row.quantity,
          row.original_price,
          row.replacement_price,
          reason,
          { publicId: row.original_public_id, name: row.original_name },
          { publicId: row.replacement_public_id, name: row.replacement_name },
          merchantOwnerPublicId,
        ],
      )
    ).rows[0];
    inserted.job_public_id = orderPublicId;
    inserted.original_public_id = row.original_public_id;
    inserted.replacement_public_id = row.replacement_public_id;
    await enqueueNotificationForInternalUser(client, {
      userId: row.customer_id,
      template: "order_substitution",
      payload: {
        jobId: orderPublicId,
        substitutionId: inserted.public_id,
        status: "pending",
        original: row.original_name,
        replacement: row.replacement_name,
      },
      deduplicationKey: `substitution:${inserted.public_id}:pending`,
    });
    ...
    if (error.code === "23505")
      throw Object.assign(new Error("Ya existe una sustitución pendiente para este producto"), {
        status: 409,
      });
    throw error;
  } finally {
...
export async function getOrderSubstitutions({ orderPublicId, actorPublicId, roles }) {
...
export async function decideOrderSubstitution({
  ...
  try {
    ...
    if (!row)
      throw Object.assign(new Error("Sustitución no encontrada, ya decidida o fuera de etapa"), {
        status: 409,
      });
    const customer = (
    ...
    if (decision === "accepted") {
      refundCents =
        (Number(row.original_unit_price_cents) - Number(row.replacement_unit_price_cents)) *
        Number(row.quantity);
      await client.query(
        `UPDATE job_items SET catalog_item_id=$2,name=$3,unit_price_cents=$4,metadata=metadata||$5::jsonb WHERE id=$1`,
        [
          row.job_item_id,
          row.replacement_catalog_item_id,
          row.replacement_snapshot.name,
          row.replacement_unit_price_cents,
          JSON.stringify({
            publicId: row.replacement_public_id,
            substitutionId: row.public_id,
      ...
      if (refundCents) {
        ...
        if (!payment || Number(payment.captured_amount_cents) < refundCents)
          throw Object.assign(new Error("No existe captura suficiente para ajustar el pedido"), {
            status: 409,
          });
        const wallet = (
            await client.query(
          ...
          transaction = (
            await client.query(
              ...
              [
                `substitution-refund-${row.public_id}`,
                customer.id,
                `Diferencia sustitución ${row.job_public_id}`,
                { substitutionId: row.public_id },
              ],
            )
          ).rows[0];
        ...
        await client.query(
          `INSERT INTO refunds(payment_intent_id,requested_by,amount_cents,reason,status,resolved_at,provider_refund_id) VALUES($1,$2,$3,'substitution','succeeded',now(),$4)`,
          [payment.id, customer.id, refundCents, row.public_id],
        );
        ...
        await client.query(
          ...
          [payment.id, remaining, remaining ? "partially_refunded" : "refunded"],
        );
      }
    }
    const updated = (
      await client.query(
        "UPDATE order_item_substitutions SET status=$2,decided_by=$3,decided_at=now(),refund_amount_cents=$4 WHERE id=$1 RETURNING *",
        [row.id, decision, customer.id, refundCents],
      )
    ).rows[0];
    Object.assign(updated, {
      job_public_id: row.job_public_id,
      original_public_id: row.original_public_id,
      replacement_public_id: row.replacement_public_id,
    });
    await client.query("COMMIT");
    return map(updated);
  } catch (error) {
