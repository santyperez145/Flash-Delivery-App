...
import {
  decryptTrustedContactPhone,
  encryptTrustedContactPhone,
  hashTrustedContactPhone,
...
const mapContact = (row) => ({
  ...
  phone: decryptTrustedContactPhone(row.phone_ciphertext),
...
export async function createPostgresTrustedContact({ userPublicId, name, relationship, phone }) {
  ...
  try {
    ...
    const phoneHash = hashTrustedContactPhone(phone),
      existing = (
        await client.query(
          "SELECT id FROM ride_trusted_contacts WHERE user_id=$1 AND phone_hash=$2",
          [user.id, phoneHash],
        )
      ).rows[0],
      count = Number(
        (
          await client.query(
            "SELECT count(*)::int count FROM ride_trusted_contacts WHERE user_id=$1 AND active",
            [user.id],
          )
        ).rows[0].count,
      );
    if (!existing && count >= 5)
      throw Object.assign(new Error("Puedes guardar hasta 5 contactos de confianza"), {
        status: 409,
      });
    const row = (
      await client.query(
...
export async function deletePostgresTrustedContact({ userPublicId, contactId }) {
  const result = await postgresPool.query(
    `DELETE FROM ride_trusted_contacts c USING users u WHERE c.id=$1 AND c.user_id=u.id AND u.public_id=$2 RETURNING c.id`,
    [contactId, userPublicId],
  );
  if (!result.rows[0])
    throw Object.assign(new Error("Contacto de confianza no encontrado"), { status: 404 });
  return getPostgresTrustedContacts(userPublicId);
}
