...
try {
  ...
  assert(anonymousDemand.status===401&&spec.paths["/api/driver/demand-zones"].get.security&&spec.components.schemas.DriverDemand.properties.methodology.properties.forecast.const===false&&spec.components.schemas.DriverDemand.properties.methodology.properties.pricingImpact.const===false,"demanda zonal exige Driver y excluye pronóstico y surge");
  assert(anonymousEarnings.status===401&&spec.paths["/api/driver/earnings"].get.security&&spec.components.schemas.DriverEarnings.required.includes("timeTracking")&&spec.components.schemas.DriverEarningsPeriod.required.includes("onlineSeconds")&&spec.components.schemas.DriverEarnings.properties.cashout.properties.status.const==="not_configured","ganancias privadas exigen identidad, publican tiempo operativo trazable y no prometen retiros sin proveedor");
