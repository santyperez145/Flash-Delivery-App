...
const originalPromotions = preferences.body.preferences.find(
  (entry) => entry.category === "promotions",
);
const updated = await call("/notification-preferences/promotions", {
  method: "PATCH",
  token: customerToken,
  body: JSON.stringify({
    pushEnabled: !originalPromotions.pushEnabled,
    emailEnabled: originalPromotions.emailEnabled,
  }),
});
...
if (
  ...
  !ownerRead.body.notifications.some((entry) => entry.id === notificationId && entry.readAt)
)
