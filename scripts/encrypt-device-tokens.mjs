...
try {
  ...
  const legacy = await client.query(
    "SELECT id,push_token FROM user_devices WHERE push_token IS NOT NULL FOR UPDATE",
  );
  for (const row of legacy.rows)
    await client.query(
      "UPDATE user_devices SET push_token_ciphertext=$2,push_token_hash=$3,push_token=NULL WHERE id=$1",
      [row.id, encryptDeviceToken(row.push_token), hashDeviceToken(row.push_token)],
    );
