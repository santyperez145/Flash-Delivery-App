...
do {
  const result = await processPostgresNotificationBatch({
    workerId: `standalone-${process.pid}`,
    provider: config.notificationProvider,
  });
  if (result.claimed)
    console.log(JSON.stringify({ level: "info", event: "notification_batch", ...result }));
  if (process.env.WORKER_ONCE === "true") break;
  await new Promise((resolve) => setTimeout(resolve, result.claimed ? 250 : 3000));
} while (!stopping);
await closePostgres();
