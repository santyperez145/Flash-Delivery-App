...
const login = async (email) =>
  (
    await call("/auth/login", {
      method: "POST",
      body: JSON.stringify({ email, password: "demo123", deviceName: "driver-demand-smoke" }),
    })
  ).body.token;
...
try {
  ...
  assert(
    (await call("/driver/demand-zones")).status === 401,
    "anonymous cannot read private driver demand",
  );
  assert(
    (await call("/driver/demand-zones", { token: customerToken })).status === 403,
    "customer cannot read driver supply signals",
  );
  const response = await call("/driver/demand-zones", { token: driverToken }),
    demand = response.body.demand;
  assert(
    response.status === 200 &&
      response.headers.get("cache-control")?.includes("no-store") &&
      demand?.source === "postgres-live-window",
    "driver receives a private live PostgreSQL snapshot",
  );
  assert(
    demand.methodology.forecast === false && demand.methodology.pricingImpact === false,
    "snapshot never claims prediction or surge pricing",
  );
  const profile = (
    await pool.query(
      "SELECT d.city_id,d.active_mode FROM drivers d JOIN users u ON u.id=d.user_id WHERE u.public_id='usr_driver'",
    )
  ).rows[0];
  ...
  const expectedById = new Map(
    expected.rows.map((row) => [
      row.public_id,
      { openJobs: Number(row.open_jobs), eligibleDrivers: Number(row.eligible_drivers) },
    ]),
  );
  assert(
    demand.zones.length === expected.rows.length &&
      demand.zones.every(
        (zone) =>
          zone.openJobs === expectedById.get(zone.id)?.openJobs &&
          zone.eligibleDrivers === expectedById.get(zone.id)?.eligibleDrivers,
      ),
    "zone counts equal authoritative jobs and eligible fresh supply",
  );
  assert(
    demand.zones.every(
      (zone) =>
        zone.boundary.length >= 4 &&
        zone.boundary.every((point) => Number.isFinite(point.lat) && Number.isFinite(point.lng)),
    ),
    "only aggregated zone geometry is exposed",
  );
  assert(
    demand.zones.every(
      (zone) =>
        zone.level ===
        (zone.openJobs >= 3 && zone.openJobs > zone.eligibleDrivers
          ? "high"
          : zone.openJobs > 0
            ? "medium"
            : "low"),
    ),
    "documented demand bands are deterministic",
  );
} finally {
  await pool.end();
}
