...
try{
 ...
 for(let attempt=0;attempt<5;attempt+=1){const wrong=await call("/me/phone-verification/confirm",{code:"000000"},true);if(wrong.status!==400)console.error("phone OTP rejection diagnostic",wrong.status,wrong.body);assert(wrong.status===400,`wrong phone OTP attempt ${attempt+1} is rejected`);}
