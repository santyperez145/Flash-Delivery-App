...
try{
  const loginResponse=await fetch(`${apiBase}/auth/login`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:"cliente@flash.app",password:"demo123",deviceName:"maps-smoke"})});
  ...
  assert(geocodeResponse.ok&&geocode.cache==="hit"&&geocode.results?.[0]?.label==="Resultado cacheado","geocoder serves durable PostgreSQL cache without provider access");
  ...
  const routeResponse=await fetch(`${apiBase}/maps/route?${params}`,{headers});const routed=await routeResponse.json();
  assert(routeResponse.ok&&routed.cache==="hit"&&routed.route?.distanceKm===6.4,"router serves durable PostgreSQL cache without provider access");
  const invalidResponse=await fetch(`${apiBase}/maps/route?fromLat=190&fromLng=0&toLat=0&toLng=0`,{headers});
  assert(invalidResponse.status===400,"router rejects out-of-range coordinates before provider access");
  const stored=JSON.stringify((await pool.query("SELECT cache_key,kind,provider FROM map_provider_cache WHERE cache_key=ANY($1)",[keys])).rows);
  assert(!stored.includes(query),"cache keys do not persist the queried address in clear text");
}finally{
  if(keys.length)await pool.query("DELETE FROM map_provider_cache WHERE cache_key=ANY($1)",[keys]);
  await pool.end();
}
