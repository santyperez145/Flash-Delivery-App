...
try {
  ...
  for (const ride of state.rides || []) {
    const customer = (
      await client.query("SELECT id FROM users WHERE public_id=$1", [ride.customerId])
    ).rows[0];
    const driver = ride.driverId
      ? (await client.query("SELECT id FROM drivers WHERE public_id=$1", [ride.driverId])).rows[0]
      : null;
    ...
    const inserted = await client.query(
      ...
      [
        ...
        ride.service,
        cents(ride.fare),
        Math.round(ride.distanceKm * 1000),
        ride.durationMin * 60,
        ride.paymentMethod,
        { etaMin: ride.etaMin, subtype: "passenger_ride" },
        ride.createdAt,
      ],
    );
    await client.query("DELETE FROM job_events WHERE job_id=$1", [inserted.rows[0].id]);
    for (const event of ride.timeline || [])
