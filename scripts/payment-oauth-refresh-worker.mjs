...
try{const result=await refreshDueMerchantPaymentConnections({limit:Number(process.env.WORKER_BATCH_SIZE)||20});console.log(JSON.stringify({worker:"payment-oauth-refresh",...result}));if(result.attention>0)process.exitCode=2;}finally{await closePostgres();}
