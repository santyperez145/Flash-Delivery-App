...
const coordinator = createAuthRefreshCoordinator(async () => {
  refreshCalls += 1;
...
let retryCalls = 0;
const retryable = createAuthRefreshCoordinator(async () => {
  retryCalls += 1;
  return retryCalls > 1;
}, () => "same-token");
...
assert.equal((app.match(/if \(loading \|\| authRequired \|\| !sessionUserId\) return;/g) || []).length, 2, "polling and realtime are both gated by authenticated UI state");
assert.match(app, /flash:auth-required/);
assert.match(app, /if \(initialBootstrapStarted\.current\) return;/);

console.log("web auth session smoke passed: refresh is single-flight, stale 401s reuse it, and anonymous polling is disabled");
