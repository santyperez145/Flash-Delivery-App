...
const call = async (path, { token, method = "GET", body } = {}) => {
  const response = await fetch(`${base}${path}`, {
    method,
    headers: {
      ...(token ? { authorization: `Bearer ${token}` } : { "x-test": "none" }),
...
const login = async (email) =>
  (
    await call("/auth/login", {
      method: "POST",
      body: { email, password: "demo123", deviceName: "analytics-smoke" },
    })
  ).payload.token;
try {
  ...
  assert(
    ...
    "La deduplicación no es idempotente",
  );
  ...
  assert(pii.response.status === 400, "Analytics aceptó una propiedad sensible");
  const forbidden = await call("/operations/product-metrics", { token: customer });
  assert(forbidden.response.status === 403, "Cliente pudo consultar agregados operativos");
  ...
  assert(
    ...
    "No se calculó el embudo",
  );
  console.log("ok - analytics first-party, dedupe, privacidad, RBAC y funnel verificados");
} finally {
  await pool.query("DELETE FROM product_events WHERE public_id=ANY($1::uuid[])", [ids]);
  api.kill("SIGTERM");
  await pool.end();
}
