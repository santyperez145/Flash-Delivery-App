...
assert(!`${app}\n${publicTracking}`.includes("buildWebTrackingMap") && !`${app}\n${publicTracking}`.includes("tile.openstreetmap.org"), "App elimina la proyección y grilla raster legacy");
...
assert(app.includes("driver={driver?.location || null}") && publicTracking.includes("driver={tracking.driver?.location || null}"), "el marcador móvil depende de coordenadas reales del backend");
assert(entry.includes('lazy(() => import("./PublicRideTrackingPage"))'), "el tracking público queda fuera de la carga inicial de la plataforma autenticada");
