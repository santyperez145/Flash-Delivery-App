...
try {
  ...
  for (const fixture of fixtures) {
    const user = (await client.query("SELECT id FROM users WHERE public_id=$1", [fixture.userId]))
      .rows[0];
    if (!user) continue;
    const existing = (
      await client.query("SELECT id FROM addresses WHERE user_id=$1 AND formatted_address=$2", [
        user.id,
        fixture.address,
      ])
    ).rows[0];
