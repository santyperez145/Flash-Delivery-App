...
const jobStatus = (status) => ({ courier_assigned: "driver_assigned", delivered: "completed" }[status] || status);

try {
  await client.query("BEGIN");
  const primaryAuth = await client.query("SELECT password_hash FROM users WHERE public_id = 'usr_driver'");
  for (const [index, driver] of state.drivers.entries()) {
    ...
    if (!user.rows[0] && primaryAuth.rows[0]) {
      user = await client.query(
        ...
        [publicUserId, `${driver.id}@seed.flash.invalid`, primaryAuth.rows[0].password_hash, driver.name,
          { driverId: driver.id, wallet: 0, defaultAddress: "" }]
      );
      await client.query("INSERT INTO user_roles(user_id, role) VALUES ($1, 'driver') ON CONFLICT DO NOTHING", [user.rows[0].id]);
    }
    if (user.rows[0]) {
      await client.query(
        ...
        [driver.id, user.rows[0].id, driver.online, driver.activeService,
          JSON.stringify(driver.serviceModes), driver.rating, driver.location.lng, driver.location.lat, {
            name: driver.name, vehicle: driver.vehicle, plate: driver.plate,
            locationLabel: driver.location.label, earningsToday: driver.earningsToday
          }]
      );
    }
  }
  ...
  for (const order of state.orders) {
    const customer = await client.query("SELECT id FROM users WHERE public_id = $1", [order.customerId]);
    const merchant = await client.query("SELECT id, location FROM merchants WHERE public_id = $1", [order.restaurantId]);
    if (!customer.rows[0] || !merchant.rows[0]) continue;
    let driverId = null;
    if (order.courierId) {
      driverId = (await client.query("SELECT id FROM drivers WHERE public_id = $1", [order.courierId])).rows[0]?.id || null;
    }
    const inserted = await client.query(
      `INSERT INTO jobs(public_id, kind, customer_id, merchant_id, driver_id, status,
       ...
       RETURNING id`,
      [order.id, customer.rows[0].id, merchant.rows[0].id, driverId, jobStatus(order.status),
        order.deliveryAddress, cents(order.total), Math.max(0, order.etaMin) * 60,
        order.paymentMethod, { subtype: "food_order", subtotal: order.subtotal,
          deliveryFee: order.deliveryFee, serviceFee: order.serviceFee, etaMin: order.etaMin,
          locationEstimated: true }, order.createdAt]
    );
    ...
    for (const item of order.items) {
      const catalog = await client.query("SELECT id FROM catalog_items WHERE public_id = $1", [item.menuItemId]);
      await client.query(
        `INSERT INTO job_items(job_id, catalog_item_id, name, quantity, unit_price_cents, customer_note, metadata)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [inserted.rows[0].id, catalog.rows[0]?.id || null, item.name, item.quantity,
          cents(item.unitPrice), item.note || null, { publicId: item.menuItemId, extras: item.extras || [] }]
      );
    }
    for (const event of order.timeline || []) {
      await client.query("INSERT INTO job_events(job_id, status, occurred_at) VALUES ($1, $2, $3)",
