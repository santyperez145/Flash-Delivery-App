...
const login=async(email)=>(await call("/auth/login",{method:"POST",body:JSON.stringify({email,password:"demo123",deviceName:"merchant-dashboard-smoke"})})).body.token;
...
try{
  ...
  const baseline=(await pool.query(`SELECT
    ...
    FROM jobs WHERE merchant_id=$1 AND kind='delivery' AND metadata->>'subtype'='food_order'`,[context.merchant_id])).rows[0];
  const inserted=await pool.query(`INSERT INTO jobs(public_id,kind,customer_id,merchant_id,branch_id,status,pickup_address,pickup_location,dropoff_address,dropoff_location,service_level,quoted_amount_cents,final_amount_cents,distance_m,estimated_duration_s,metadata,merchant_prep_minutes,merchant_ready_due_at)
    ...
    UNION ALL SELECT $1||'-CANCEL','delivery'::job_kind,$2::uuid,$3::uuid,$4::uuid,'cancelled'::job_status,b.address,b.location,'Destino',b.location,'food',180000,180000,1000,1800,'{"subtype":"food_order"}'::jsonb,15::smallint,now()-interval '5 minutes' FROM merchant_branches b WHERE b.id=$4::uuid RETURNING id,status`,[prefix,context.customer_id,context.merchant_id,context.branch_id]);
  for(const job of inserted.rows)await pool.query("INSERT INTO job_events(job_id,status) VALUES($1,$2)",[job.id,job.status]);
