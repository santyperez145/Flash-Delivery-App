...
7. Limitar cada archivo a una responsabilidad concreta. **Avance:** horarios/modificadores → `MerchantCatalogEditors`; finanzas admin → `AdminFinancePanels`; envíos admin → `AdminShipmentPanels`; soporte admin → `AdminSupportPanels` (`AdminConsole` ~1.330). Guía/firma → `DriverDeliveryModals`; cuenta y ganancias del conductor → `DriverAccountPanel` / `DriverEarningsPanel` (`DriverScreen` ~1.320, bajo el techo de 1.500).
