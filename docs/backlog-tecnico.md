...
- [x] **Ningún `App.tsx` supera 1.500 líneas.** `apps/mobile/App.tsx` 15.374 → **321**; `src/App.tsx` 10.553 → **1.274**. En los dos queda sólo el shell.
...
- [ ] Los grants dejan de ser `ON ALL TABLES`. **Avanzado**: la migración 116 revocó la escritura sobre las 8 tablas de referencia donde el runtime nunca escribe y retiró la herencia automática, que era lo que hacía nacer con DML a toda tabla nueva. `test:grant-scope` impide reintroducir cualquiera de las dos formas. Desde el 27-08 el trabajo dejó de ser una estimación: `test:runtime-write-scope` cruza permisos, writes del código y writes de rebote por trigger, y midió **114 permisos de más en 86 tablas** —pares (tabla, operación), que es como pide el criterio—. La migración 122 acotó 16, los de dinero y registros de eventos; la 123 revocó 26 más, los registros de hechos ocurridos, incluido el caso más grave del inventario: **el runtime tenía INSERT, UPDATE y DELETE sobre `schema_migrations`**, el registro de qué migraciones se aplicaron. La 131 elimina otros 11 DELETE innecesarios sobre sesiones, MFA, dispositivos, desafíos, safety y autorizaciones efímeras. La 132 retira otros 6 DELETE sobre webhooks, conexión PSP, conciliación, payouts y evaluaciones de riesgo. La 133 protege diez registros del historial operativo: jobs, ofertas, incidencias, sustituciones, notificaciones, evidencias, reclamos y devoluciones. La 134 revoca nueve operaciones administrativas: UPDATE de roles e INSERT/DELETE de flags, comercios, zonas y perfiles de soporte; los últimos cuatro lotes se prueban por nombre y operación contra PostgreSQL, no sólo por conteo. La cuenta queda en **35 permisos excedentes**, trinquetada para que no pueda subir.
