...
6. Crear contratos compartidos en un paquete propio. **Avance:** `@flash/domain-contracts` concentra **49** tipos (`PromotionSummary`/`VerticalService` + Ride/Shipment/`DriverVehicle`/dashboard/menú/`Order`). Web: `Service = VerticalService`, `Promotion = PromotionSummary`; mobile añade ventana de promo. `test:domain-contracts` bloquea el merge. Extras del comercio siguen locales.
