...
7. Limitar cada archivo a una responsabilidad concreta. **Avance:** finanzas → `AdminFinancePanels`; envíos → `AdminShipmentPanels`; soporte → `AdminSupportPanels`; confianza → `AdminTrustPanels`; tableros overview → `AdminOverviewBoards` (`AdminConsole` ~582, orquestación de secciones). Guía/firma → `DriverDeliveryModals`; cuenta/ganancias → `DriverAccountPanel` / `DriverEarningsPanel` (`DriverScreen` ~1.298).
