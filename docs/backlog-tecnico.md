...
7. Limitar cada archivo a una responsabilidad concreta. **Avance:** finanzas → `AdminFinancePanels`; envíos → `AdminShipmentPanels`; soporte → `AdminSupportPanels`; legajo/moderación/MFA → `AdminTrustPanels` (`AdminConsole` ~871). Guía/firma → `DriverDeliveryModals`; cuenta/ganancias → `DriverAccountPanel` / `DriverEarningsPanel` (`DriverScreen` ~1.298).
