...
- [ ] Las tablas de la deuda tienen política y prueba negativa. **Va una de cinco**: `shipment_details` cerró el 27-08 con `113_shipment_details_rls.sql` y las dos mitades de la prueba en `test:rls` —el auditor con grant ve cero filas, y un cliente no ve destinatarios ajenos—. Quedan `user_roles`, que exige mover el login a `SECURITY DEFINER` primero, `drivers`, `merchants` y `promotion_redemptions`.
