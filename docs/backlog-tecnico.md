...
- [x] **Las cinco tablas de la deuda tienen política y prueba negativa.** Cerraron el 27-08 y `test:rls-matrix` reporta **65 de 65**. Se repitió un patrón que conviene registrar: en tres de las cinco —`drivers`, `merchants`, `user_roles`— el motivo anotado para no aplicarlas era más restrictivo que el esquema real. Bloqueaban por consultas sin contexto de usuario, y eso sólo bloquea si la política alcanzara al runtime, que queda exento como en las otras 67 tablas.
