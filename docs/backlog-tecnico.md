...
- [ ] Los grants dejan de ser `ON ALL TABLES`. **Avanzado**: la migración 116 revocó la escritura sobre las 8 tablas de referencia donde el runtime nunca escribe y **retiró la herencia automática**, que era lo que hacía nacer con DML a toda tabla nueva —la causa de que un almacén de credenciales muerto quedara alcanzable—. `test:grant-scope` impide reintroducir cualquiera de las dos formas. Falta acotar por operación las 96 tablas restantes, que exige decidir tabla por tabla.
...
- [x] **Existe SBOM y scan de imagen en el pipeline.** El job `container-image` genera un SBOM CycloneDX que publica como artefacto, y escanea con Trivy por imagen fijada —no por una acción del marketplace, que sería código de terceros con el token del workflow—. **Bloquea sólo lo que tiene arreglo disponible**: una base acumula CVEs sin parche upstream de forma permanente, y bloquear por ésas cierra el merge por algo irresoluble hasta que alguien desactiva la puerta. Lo no arreglable se informa sin cortar.
