...
- [ ] Las tablas de la deuda tienen política y prueba negativa. **Van dos de cinco**: `shipment_details` y `promotion_redemptions` cerraron el 27-08. La segunda destapó que nada afirmaba el cupo global de una promoción: la verificación existente leía con el rol dueño, que saltea RLS. Quedan `user_roles` —exige mover el login a `SECURITY DEFINER`—, `drivers` y `merchants`, que requieren auditar 39 y 23 archivos que las consultan sin contexto de usuario.
