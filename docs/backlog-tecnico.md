...
7. Limitar cada archivo a una responsabilidad concreta. **Avance:** finanzas → `AdminFinancePanels`; envíos → `AdminShipmentPanels`; soporte → `AdminSupportPanels`; confianza → `AdminTrustPanels`; overview → `AdminOverviewBoards` (`AdminConsole` ~582). Guía/firma → `DriverDeliveryModals`; cuenta/ganancias → `DriverAccountPanel` / `DriverEarningsPanel`; inbox → `DriverInboxPanel`; tarjetas viaje/envío → `DriverJobCards` (`DriverScreen` ~1.098).
