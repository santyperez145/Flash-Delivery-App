...
5. ~~Dividir `commerce-repository.js` por subdominio~~ **Hecho**: `catalog-repository.js` (~401, restaurantes), `menu-repository.js` (carta/modificadores/dieta), `branch-repository.js` (sucursales/horarios/stock), `order-repository.js` (~770), `order-quote-repository.js`, `order-marketplace-payment-repository.js`, `cart-repository.js` + `order-selection.js` y `driver-roster-repository.js`. `usesPostgresCommerce` vive en `postgres.js`. Pedidos/carrito importan `mapCatalogItem` de catálogo —nunca al revés.
6. Crear contratos compartidos en un paquete propio. **Avance:** `@flash/domain-contracts` concentra **52** tipos (`DriverEarnings*` + `PromotionSummary`/`VerticalService` + Ride/Shipment/`DriverVehicle`/dashboard/menú/`Order`). Web: `Service = VerticalService`, `Promotion = PromotionSummary`; mobile añade ventana de promo. `test:domain-contracts` bloquea el merge. Extras del comercio siguen locales.
