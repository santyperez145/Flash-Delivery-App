...
2. Extraer features de los dos `App.tsx` hacia módulos por dominio. **Parcial**: mobile quedó en 321 líneas y web en 1.286 al mover el acceso real a `auth/WebLogin.tsx` (170 líneas) y convertir Customer, Merchant, Operaciones y Superadmin en límites de carga por audiencia. El entry web bajó de 575,7 a 65,7 KiB. La primera extracción interna del cliente movió Actividad —grupos, sustituciones, servicios activos, recibos, repetición, reclamos y propinas— a `CustomerActivityScreen.tsx` (571 líneas) sin alterar sus operaciones reales. `CustomerScreen.tsx` bajó de 6.241 a 5.965 líneas y sigue siendo el archivo más grande del repositorio; Comidas, Viajes, Envíos y Cuenta son los próximos cortes.
