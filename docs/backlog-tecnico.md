...
6. Crear contratos compartidos en un paquete propio. **Avance:** `@flash/domain-contracts` concentra 20 tipos byte-a-byte idénticos y autocontenidos entre web y mobile; ambas superficies los reexportan y `test:domain-contracts` bloquea el merge. `MerchantOperationsDashboard` sigue local porque referencia `Restaurant`, que diverge. User, Order, Restaurant y el resto quedan para cortes posteriores.
7. Limitar cada archivo a una responsabilidad concreta. **Avance:** horarios, modificadores y declaración alimentaria salieron de `MerchantConsole.tsx` a `MerchantCatalogEditors.tsx`. Tarifas, payouts, propinas y conciliación salieron de `AdminConsole.tsx` (~3.216 → ~2.400) a `AdminFinancePanels.tsx` (~841). Guía giro a giro y firma de recepción salieron de `DriverScreen.tsx` (~2.471 → ~2.155) a `DriverDeliveryModals.tsx` (~342). Siniestros y configuración de categorías/SLA salieron de `AdminConsole.tsx` (~2.381 → ~1.879) a `AdminShipmentPanels.tsx` (~511).
...
- [x] Un PR queda bloqueado si falla cualquier suite crítica — **106 de 107 suites con puerta, 104 bloqueantes**, 2 nocturnas y cuarentena vacía.
