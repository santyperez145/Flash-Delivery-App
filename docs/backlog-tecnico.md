...
- [ ] Las tablas de la deuda tienen política y prueba negativa. **Van cuatro de cinco**: `shipment_details`, `promotion_redemptions`, `drivers` y `merchants` cerraron el 27-08. El motivo registrado para las dos últimas —«39 y 23 archivos sin contexto»— no resistió el inventario: sólo bloquearía si la política alcanzara al runtime, que queda exento como en las otras 66 tablas. Lo que sí estaba abierto era que el rol auditor tenía `GRANT SELECT` sobre ambas sin `ENABLE`. Queda `user_roles`, que exige mover el login a `SECURITY DEFINER` primero.
