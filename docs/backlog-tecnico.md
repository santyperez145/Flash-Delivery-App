...
6. Crear contratos compartidos en un paquete propio. **Avance:** `@flash/domain-contracts` concentra 31 tipos compartidos (`RestaurantSummary` sumado al lote de 30 con `OrderItem`, núcleo `Order`, `User`, estados y 27 más). `OrderItem` es estricto (campos del servidor); web extiende `Order` con timeline/tarifas y mobile con `cancellation`. `RestaurantSummary` fija la intersección web/mobile (id, owner, catálogo visual, ETA/tarifa); cada superficie extiende `Restaurant` con menú/sucursales (`MenuItem`/`extras`/`lat`/`lng` web vs menú inline mobile). Ambas reexportan y `test:domain-contracts` bloquea el merge. `MerchantOperationsDashboard` sigue local porque referencia `Restaurant` completo.
