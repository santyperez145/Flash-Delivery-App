...
4. ~~Descomponer `server/index.js`~~ **Hecho**: 57 grupos de rutas en routers bajo `server/http/` (incl. `platform-status`, `readiness`, `bootstrap`, `metrics`). El archivo quedó en **364 líneas**: arranque, middleware, montaje y `POST /api/reset`.
...
6. Crear contratos compartidos en un paquete propio. **Avance:** `@flash/domain-contracts` concentra **39** tipos (`DriverVehicle` sumado a `DriverSummary`/dashboard/menú/sucursales/`Order`/`User`). Web: `Driver = DriverSummary`; mobile extiende conductor con KYC/GPS. `test:domain-contracts` bloquea el merge. Extras del comercio siguen locales.
...
- [x] **`server/index.js` deja de contener lógica de dominio.** 57 de 57 grupos de rutas extraídos a routers en `server/http/`. `index.js` bajó de 9.696 a **364 líneas** (−96%): arranque, middleware, montaje y el reset de demo SQLite. Salud, readiness, OpenAPI, bootstrap, métricas y el 410 de `/api/state` viven en routers de infraestructura.
