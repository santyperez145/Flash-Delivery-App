...
- [ ] Ninguna línea de más de 200 caracteres. Quedan **224** (bajan de 251): se partieron SQL en `tip-repository`, `zone-readiness-repository`, `risk-repository`, `catalog-repository` y la descripción larga de `openapi.js`. El resto son SQL en template literals; `test:line-length` fija el nuevo techo. DAT-001 ya no bloquea este trabajo.
