...
7. Limitar cada archivo a una responsabilidad concreta. **Avance:** horarios, modificadores y declaración alimentaria salieron de `MerchantConsole.tsx` a `MerchantCatalogEditors.tsx`. Tarifas, payouts, propinas y conciliación → `AdminFinancePanels.tsx`. Guía/firma del conductor → `DriverDeliveryModals.tsx`. Siniestros y categorías/SLA → `AdminShipmentPanels.tsx`. Respuestas rápidas, dead letters y operaciones de soporte/SLA → `AdminSupportPanels.tsx`; `AdminConsole.tsx` queda ~1.365 líneas (bajo el techo de 1.500 del criterio ARC-001).
