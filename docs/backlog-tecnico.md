...
- [ ] El build de driver no incluye pantallas de comercio. **Destrabado**: `App.tsx` elegía la pantalla por prioridad de rol e ignoraba la variante instalada, así que cada build tenía que incluir las tres. Con el modo fijado a `mobileAppVariant`, cada entrypoint necesita exactamente una. Falta partir `index` por variante.
- [ ] El build de customer no incluye backoffice. Mismo destrabe que el criterio anterior; hoy los tres bundles cargan las 9.715 líneas de las tres pantallas.
- [ ] `server/index.js` deja de contener lógica de dominio. **31 de 57 grupos de rutas extraídos**; quedan 97 rutas en 26 grupos. `index.js` bajó de 9.696 a 5042 líneas. El ciclo del pedido y sus incidencias salieron con tres módulos compartidos nuevos: `driver-earnings.js` (lo comparten pedidos, viajes y envíos), `http/cancellation.js` (el motivo de cancelación se compara entre servicios) y `addTimeline` en `fallback-runtime.js`.
