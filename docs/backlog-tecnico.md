...
6. Crear contratos compartidos en un paquete propio. **Avance:** `@flash/domain-contracts` concentra 30 tipos compartidos (`OrderItem` y el núcleo `Order` sumados al lote previo con `User`, estados y 27 más). `OrderItem` es estricto (campos del servidor); web extiende con timeline/tarifas y mobile con `cancellation`. Ambas superficies reexportan y `test:domain-contracts` bloquea el merge. `Restaurant` sigue local: menú inline vs `MenuItem`/`extras`, `lat`/`lng` sólo web y sucursales tipadas distinto — `RestaurantSummary` se pospone este corte. `MerchantOperationsDashboard` sigue local porque referencia `Restaurant` completo.
