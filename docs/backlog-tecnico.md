...
6. Crear contratos compartidos en un paquete propio. **Avance:** `@flash/domain-contracts` concentra **35** tipos (`MenuItemSummary`, `MenuModifier`/`MenuModifierGroup`, `RestaurantBranch` sumados al lote con `RestaurantSummary`, `Order`/`OrderItem`, `User` y estados). Web extiende `MenuItem = MenuItemSummary &` con vitrina; mobile usa `menu: MenuItemSummary[]` y ambas `branches?: RestaurantBranch[]`. `test:domain-contracts` bloquea el merge. Extras del comercio y `MerchantOperationsDashboard` siguen locales.
...
- [x] Un PR queda bloqueado si falla cualquier suite crítica — **106 de 108 suites con puerta, 104 bloqueantes**, 2 nocturnas y cuarentena vacía.
