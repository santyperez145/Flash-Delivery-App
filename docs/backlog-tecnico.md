...
- [ ] `server/index.js` deja de contener lógica de dominio. **54 de 57 grupos de rutas extraídos**; quedan 15 rutas en 10 grupos. `index.js` bajó de 9.696 a 1343 líneas (−86%). La cuenta propia y su saldo salieron juntas porque comparten `accountSnapshot`: son el mismo objeto en tres momentos. De lo que resta, ocho rutas son infraestructura del servidor —salud, readiness, OpenAPI, bootstrap, métricas, estado y reset— y deberían quedarse.
