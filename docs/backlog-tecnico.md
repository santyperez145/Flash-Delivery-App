...
- [ ] `server/index.js` deja de contener lógica de dominio. **20 de 57 grupos de rutas extraídos**; quedan 147 rutas en 35 grupos. `index.js` bajó de 9.696 a 7.042 líneas. El grupo `admin` no se extrae entero porque no es un dominio sino una audiencia: hoy mezcla gobernanza tarifaria, moderación de cuentas, conciliación financiera, revisión de documentos y disparadores de workers. Se va cortando por ciclo de vida, y el primer corte fue tarifas.
