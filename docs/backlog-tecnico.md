...
5. ~~Dividir `commerce-repository.js` por subdominio~~ **Hecho**: `catalog-repository.js` (~401, restaurantes), `menu-repository.js` (carta/modificadores/dieta), `branch-repository.js` (sucursales/horarios/stock), `order-repository.js` (~143 lecturas), `order-create-repository.js` (~550 alta), `order-lifecycle-repository.js` (~93 avance/asignación), `order-quote-repository.js`, `order-marketplace-payment-repository.js`, `cart-repository.js` + `order-selection.js` y `driver-roster-repository.js`. `usesPostgresCommerce` vive en `postgres.js`. Pedidos/carrito importan `mapCatalogItem` de catálogo —nunca al revés.
