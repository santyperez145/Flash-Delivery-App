...
| 134 migraciones versionadas | `CI` | `ci-postgres.yml` corre desde cero y de forma incremental sobre la rama base |
...
| Alcance de permisos del runtime | `CI` | `test:grant-scope` y `test:runtime-write-scope`. Las migraciones hasta la 134 reducen los permisos DML sin uso de 114 a 35; verifican por nombre artefactos de identidad, evidencia financiera, historial operativo y autoridad administrativa acotada por operación |
