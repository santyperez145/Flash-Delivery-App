...
| Contratos compartidos web/móvil | `CI` | `@flash/domain-contracts` con 52 tipos (incluye `DriverEarnings*`, `PromotionSummary`, `RideSummary`, `ShipmentSummary`, `DriverVehicle`, `Order`/`User`); `test:domain-contracts` exige paquete, reexport y ausencia de cuerpos duplicados |
