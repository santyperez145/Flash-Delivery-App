...
| Contratos compartidos web/móvil | `CI` | `@flash/domain-contracts` con 35 tipos (incluye `MenuItemSummary`, `RestaurantBranch`, `RestaurantSummary`, `OrderItem`, núcleo `Order`, `User`/estados); web extiende Order/MenuItem y Restaurant con extras/coords; mobile usa menú núcleo y mismas sucursales; `test:domain-contracts` exige paquete, reexport y ausencia de cuerpos duplicados |
