...
| Contratos compartidos web/móvil | `CI` | `@flash/domain-contracts` con 30 tipos (incluye `OrderItem`, núcleo `Order`, `User`/`UserRole`/`OrderStatus`/`RideStatus`); web extiende Order con timeline/tarifas, mobile con cancelación; `test:domain-contracts` exige paquete, reexport y ausencia de cuerpos duplicados. `Restaurant` sigue divergente |
