...
| Contratos compartidos web/móvil | `CI` | `@flash/domain-contracts` con 36 tipos (incluye `MerchantOperationsDashboardCore`, `MenuItemSummary`, `RestaurantBranch`, `RestaurantSummary`, `OrderItem`, núcleo `Order`, `User`/estados); web/mobile extienden dashboard/menú; `test:domain-contracts` exige paquete, reexport y ausencia de cuerpos duplicados |
