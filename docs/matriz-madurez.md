...
| Contratos compartidos web/móvil | `CI` | `@flash/domain-contracts` con 38 tipos (incluye `DriverSummary`, `MerchantOperationsDashboardCore`, `MenuItemSummary`, `RestaurantBranch`, `Order`/`User`); `test:domain-contracts` exige paquete, reexport y ausencia de cuerpos duplicados |
