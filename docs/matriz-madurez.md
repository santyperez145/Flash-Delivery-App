...
| Contratos compartidos web/móvil | `CI` | `@flash/domain-contracts` con 31 tipos (incluye `RestaurantSummary`, `OrderItem`, núcleo `Order`, `User`/`UserRole`/`OrderStatus`/`RideStatus`); web extiende Order con timeline/tarifas y Restaurant con `MenuItem`/`extras`/`lat`/`lng`, mobile con cancelación y menú inline; `test:domain-contracts` exige paquete, reexport y ausencia de cuerpos duplicados |
