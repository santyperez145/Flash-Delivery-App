...
export const api = {
  ...
  async state() {
    const [
      ...
      activeAudience === "merchant" ? Promise.resolve(null) : this.getCatalog(undefined, 50),
  ...
  async createSandboxPaymentMethod(input: {
    ...
    return request<{ paymentMethod: import("./types").PaymentMethod }>("/payment-methods/sandbox", {
      method: "POST",
  ...
  async revokeOtherAccountSessions() {
    if (!refreshToken) throw new Error("La sesión actual no tiene credencial de renovación");
