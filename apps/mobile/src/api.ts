...
export const api = {
  ...
  async getAccountContext(){return request<{account:{user:import("./types").User;addresses:import("./types").UserAddress[];paymentMethods:import("./types").PaymentMethod[];supportTickets:import("./types").SupportTicket[];tips:import("./types").ServiceTip[];favoriteRestaurantIds:string[]}}>("/me");},
  ...
  async getPromotions(){return request<{promotions:import("./types").Promotion[]}>("/promotions");},
  async setFavorite(restaurantId:string,favorite:boolean){return request<{restaurantIds:string[]}>(`/favorites/${restaurantId}`,{method:"PUT",body:JSON.stringify({favorite})});},
  ...
  async createSupportTicket(input:{category:"food"|"ride"|"shipment"|"payment"|"account"|"safety"|"other";priority:"low"|"normal"|"high"|"urgent";subject:string;body:string;jobId?:string}){return request<{ticket:import("./types").SupportTicket}>("/support/tickets",{method:"POST",headers:{"Idempotency-Key":`mobile-support-${Date.now()}-${Math.random().toString(36).slice(2)}`},body:JSON.stringify(input)});},
  ...
  async sendSupportMessage(ticketId:string,body:string){return request<{ticket:import("./types").SupportTicket}>(`/support/tickets/${ticketId}/messages`,{method:"POST",headers:{"Idempotency-Key":`mobile-support-message-${Date.now()}-${Math.random().toString(36).slice(2)}`},body:JSON.stringify({body,internal:false})});},
