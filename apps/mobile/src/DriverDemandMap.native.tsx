...
function regionFor(points: GeoPoint[]): Region {
  const latitudes = points.map((point) => point.lat),
    longitudes = points.map((point) => point.lng);
  ...
  return {
    ...
    latitudeDelta: Math.max((maxLatitude - minLatitude) * 1.28, 0.025),
    longitudeDelta: Math.max((maxLongitude - minLongitude) * 1.28, 0.025),
  };
}
...
function center(boundary: GeoPoint[]): GeoPoint {
  const points =
    boundary.length > 1 &&
    boundary[0].lat === boundary.at(-1)?.lat &&
    boundary[0].lng === boundary.at(-1)?.lng
      ? boundary.slice(0, -1)
      : boundary;
  return {
...
export default function DriverDemandMap({
  ...
  const validZones = useMemo(
    () =>
      zones
        .map((zone) => ({ ...zone, boundary: zone.boundary.filter(valid) }))
        .filter((zone) => zone.boundary.length >= 4),
    [zones],
  );
  const visiblePoints = useMemo(
    () => [...validZones.flatMap((zone) => zone.boundary), ...(valid(driver) ? [driver] : [])],
    [driver, validZones],
  );
  const coordinates = useMemo(() => visiblePoints.map(coordinate), [visiblePoints]);
  const initialRegion = useMemo(
    () => (visiblePoints.length ? regionFor(visiblePoints) : null),
    [visiblePoints],
  );
  const fit = useCallback(
    (animated = true) => {
      ...
      mapRef.current.fitToCoordinates(coordinates, {
        animated,
        edgePadding: { top: 78, right: 22, bottom: 58, left: 22 },
      });
    },
    [coordinates],
  );
  ...
  if (!initialRegion)
    return (
      <View
        style={[styles.shell, styles.fallback, { height }]}
        accessibilityLabel={accessibilityLabel}
      >
        <Ionicons name="map-outline" size={28} color="#7c3cff" />
        <Text style={styles.fallbackTitle}>Sin zonas cartográficas habilitadas</Text>
        <Text style={styles.fallbackText}>
          Operaciones debe publicar polígonos PostGIS activos para esta ciudad.
        </Text>
      </View>
    );
  if (
    ...
    return (
      ...
      >
        <Ionicons name="map-outline" size={28} color="#7c3cff" />
        <Text style={styles.fallbackTitle}>Mapa Android pendiente de configuración</Text>
        <Text style={styles.fallbackText}>
          Configurá la clave restringida de Google Maps y generá un nuevo build. Los datos zonales
  ...
  return (
    <View style={[styles.shell, { height }]} accessibilityLabel={accessibilityLabel}>
      <MapView
        ...
        mapPadding={{ top: 72, right: 12, bottom: 48, left: 12 }}
        pitchEnabled={false}
        rotateEnabled={false}
        showsCompass={false}
        showsScale={false}
      ...
      >
        {validZones.map((zone) => (
          <Polygon
            key={zone.id}
            coordinates={zone.boundary.map(coordinate)}
            fillColor={colors[zone.level].fill}
            strokeColor={colors[zone.level].stroke}
            strokeWidth={zone.containsDriver ? 3 : 2}
          />
        ))}
        {validZones.map((zone) => (
          <Marker
            key={`${zone.id}-label`}
            coordinate={coordinate(center(zone.boundary))}
            tracksViewChanges={false}
            anchor={{ x: 0.5, y: 0.5 }}
          >
            <View style={[styles.zoneLabel, { borderColor: colors[zone.level].stroke }]}>
              <View style={[styles.zoneDot, { backgroundColor: colors[zone.level].dot }]} />
      ...
      <View style={styles.legend} pointerEvents="none">
        {(
          ...
          <View style={styles.legendItem} key={level}>
            <View style={[styles.legendDot, { backgroundColor: colors[level].dot }]} />
      ...
      <Pressable
        ...
        accessibilityLabel="Reencuadrar zonas"
      >
...
const styles = StyleSheet.create({
  ...
  legendText: { color: "#5f5964", fontSize: 8, fontWeight: "800" },
  recenter: {
    position: "absolute",
    right: 10,
    bottom: 10,
  ...
  zoneText: { flexShrink: 1, color: "#27212b", fontSize: 9, fontWeight: "900" },
  driverMarker: {
    width: 38,
    height: 38,
    borderRadius: 19,
  ...
  fallback: { alignItems: "center", justifyContent: "center", gap: 7, padding: 24 },
  fallbackTitle: { color: "#17131c", fontSize: 13, fontWeight: "900", textAlign: "center" },
  fallbackText: {
    color: "#716a76",
    fontSize: 11,
    fontWeight: "600",
