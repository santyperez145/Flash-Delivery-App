...
export type DriverDemand={driverId:string;service:"delivery"|"ride"|"shopping";online:boolean;city:{id:string;slug:string;name:string;timezone:string};observedAt:string;source:"postgres-live-window";methodology:{openJobs:"dispatchable_unassigned";scheduledHorizonMinutes:15;supplyFreshnessMinutes:5;maximumLocationAccuracyM:100;forecast:false;pricingImpact:false};zones:DriverDemandZone[]};

export type MerchantOperationsMetrics={activeOrders:number;needsAction:number;preparing:number;readyForPickup:number;courierFlow:number;lateOrders:number;untrackedPrepOrders:number;oldestActiveMinutes:number;completedToday:number;cancelledToday:number;grossSalesToday:number;averageTicketToday:number;unavailableItems:number};
export type MerchantOperationsDashboard={generatedAt:string;source:"postgres-live-operations"|"sqlite-test-fallback";timezone:string;restaurantId:string;branch:null|{id:string;name:string;timezone:string;open:boolean;manualOpen:boolean;status:"active"|"paused"|"closed";etaMin:number};restaurant:Restaurant;metrics:MerchantOperationsMetrics};
