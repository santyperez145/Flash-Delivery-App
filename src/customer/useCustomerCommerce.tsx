// Sesión de comercio del cliente web (ARC-001).
//
// Uber Eats separa catálogo, carrito, checkout firmado y cotización de viaje
// del shell de plataforma. Flash deja esa máquina de estado aquí; App.tsx
// sólo arranca sesión y enruta por audiencia.
...
type RunAction = (action: () => Promise<unknown>, success: string) => Promise<void> | void;
...
export function useCustomerCommerce({
  ...
  useEffect(() => {
    if (!activeUser?.roles.includes("customer")) return;
    let cancelled = false;
    void Promise.all([api.cart(), api.getDietaryPreferences()])
      .then(([saved, dietary]) => {
        if (cancelled) return;
        setCart(saved.cart);
