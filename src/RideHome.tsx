...
const services: Array<{ id: Ride["service"]; label: string; icon: typeof Car }> = [
...
export default function RideHome({
  ...
  const savedDestinations = addresses.filter(
    (entry) =>
      entry.lat !== null &&
      entry.lng !== null &&
      (entry.address !== rideForm.pickup ||
        entry.lat !== rideForm.pickupCoords?.lat ||
        entry.lng !== rideForm.pickupCoords?.lng),
  );
  useEffect(() => {
    let active = true;
    setRoute(null);
    setRouteError("");
    if (!rideForm.pickupCoords || !rideForm.destinationCoords) return;
    api
      .route(rideForm.pickupCoords, rideForm.destinationCoords)
      .then((result) => {
        if (active) setRoute(result.route);
      })
      .catch((error) => {
        if (active) setRouteError(error instanceof Error ? error.message : "Ruta no disponible");
      });
    return () => {
      active = false;
    };
    ...
    <>
      {hasMap ? (
        <Suspense
          fallback={
            <section
              className="ride-map ride-preview-map flash-map-loading"
        ...
        >
          <FlashMap
            origin={rideForm.pickupCoords!}
            destination={rideForm.destinationCoords!}
            route={route?.coordinates || []}
            className="ride-map ride-preview-map"
            ariaLabel="Vista previa interactiva del recorrido"
            caption={
              route
                ? `${route.distanceKm} km · ${route.durationMin} min`
                : routeError || "Calculando ruta vial…"
            }
            detail={
              routeError
                ? "La cotización informará si existe cobertura."
                : "Arrastrá para explorar · usá el control para reencuadrar"
            }
          />
        </Suspense>
      ) : (
        <section
          className="ride-map ride-map-empty"
          aria-label="Definí origen y destino para ver el mapa"
        >
          <div className="ride-map-empty-copy">
            <LocateFixed size={24} />
            <strong>Tu viaje empieza en una ubicación real</strong>
            <span>Usá el GPS o una dirección guardada y elegí el destino.</span>
          </div>
        </section>
      )}
      <section className="booking-card">
        <label>
          ...
          <input
            ...
            placeholder="Usá GPS o escribí el punto de partida"
            onChange={(event) =>
              setRideForm((current) => ({
                ...current,
        ...
        >
          <LocateFixed size={15} />
          {locationStatus === "locating" ? "Buscando GPS..." : "Usar mi ubicación actual"}
        </button>
        {locationMessage && (
          <small className={`location-message ${locationStatus}`}>{locationMessage}</small>
        )}
        {savedDestinations.length > 0 && (
          <div className="ride-saved-destinations" aria-label="Destinos guardados">
              ...
              <button
                ...
                onClick={() =>
                  setRideForm((current) => ({
                    ...
                    destinationCoords: { lat: entry.lat!, lng: entry.lng! },
                  }))
                }
              >
                <MapPin size={14} />
        ...
        <div className="two-actions">
          <button
            ...
            onClick={quoteRide}
            type="button"
            disabled={busy || !rideForm.pickup.trim() || !rideForm.destination.trim()}
          >
      ...
      {activeRide && (
        <section className="tracking-card">
          <div>
            ...
            <strong>
              {statusLabel[activeRide.status]} · {activeRide.etaMin} min
            </strong>
            <small>
          ...
          <div className="stepper">
            {steps.map((step, index) => {
              const currentIndex = steps.indexOf(activeRide.status);
              return (
          ...
          <div className="tracking-person">
            ...
            <div>
              <strong>{driver?.name || "Asignando conductor"}</strong>
              <small>{money.format(activeRide.fare)}</small>
            </div>
          </div>
        </section>
      )}
